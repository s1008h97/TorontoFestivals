...
import com.toronto.festivals.jk.adapter.FestivalsAdapter
...
class FestivalsFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        context?.let {
            binding =
                DataBindingUtil.inflate(inflater, R.layout.fragment_festivals, container, false)
            ...
            val adapter = RollingViewPagerAdapter(bannerImgList, it)
            binding.rollingViewPager.adapter = adapter

            festivalsViewModel.startAutoScroll(binding.rollingViewPager)

            val festivalsTestAdapter = FestivalsAdapter(bannerImgList)

            binding.recyclerView.adapter = festivalsTestAdapter
